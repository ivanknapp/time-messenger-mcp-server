...
`emoji_name` принимает каноническое имя эмодзи (`+1`, `100`), запись с
двоеточиями (`:tada:`), распространённый алиас (`thumbsup`) или сам символ
(`👍`). Алиасы и символы приводятся к каноническому имени — иначе реакция
встанет отдельной от той, что ставит нативный клиент: в API они уникальны по
паре пользователь + имя эмодзи.
